namespace EasyPeasyFirstPersonController
{
    ...
    public partial class FirstPersonController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            bool wantsToCrouch = canCrouch && Input.GetKey(KeybindManager.Instance.GetKey("Crouch")) && !isSliding;
            ...
            if (canSlide && isSprinting && Input.GetKeyDown(KeybindManager.Instance.GetKey("Crouch")) && isGrounded)
            {
                isSliding = true;
        ...
        private void HandleMovement()
        {
            ...
            if (Input.GetKey(KeybindManager.Instance.GetKey("Forward"))) { moveInput.y = 1; }
            if (Input.GetKey(KeybindManager.Instance.GetKey("Backward"))) { moveInput.y = -1; }
            if (Input.GetKey(KeybindManager.Instance.GetKey("Left"))) { moveInput.x = -1; }
            if (Input.GetKey(KeybindManager.Instance.GetKey("Right"))) { moveInput.x = 1; }

            isSprinting = canSprint && Input.GetKey(KeybindManager.Instance.GetKey("Sprint")) && moveInput.y > 0.1f && isGrounded && !isCrouching && !isSliding;
            ...
            {
                if (canJump && Input.GetKeyDown(KeybindManager.Instance.GetKey("Jump")) && !isSliding)
                {
        ...
        {
            isLook = false;
            StartCoroutine(LookAtTarget(target, 0.5f));
        }

        private IEnumerator LookAtTarget(Transform target, float duration)
        {
